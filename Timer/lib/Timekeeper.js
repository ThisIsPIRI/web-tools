 ...
 * @param mode {Number} Either Timekeeper.Mode.TIMER or Timekeeper.Mode.STOPWATCH.*/
const Timekeeper = function(onstop, updateCallback, format, showH, showOne, mode) {
	...
	this.mode = mode != undefined ? mode : Timekeeper.Mode.TIMER;
};
...
Timekeeper.prototype.update = function() {
	...
	if(this.mode === Timekeeper.Mode.CLOCK) {
		const date = new Date();
		...
		if(this.mode === Timekeeper.Mode.STOPWATCH)
			this.time += elapsed;
		else
			this.time -= elapsed;
		if(this.mode === Timekeeper.Mode.TIMER && this.time <= 0) {
			this.stop(true);
...
Timekeeper.prototype.start = function() {
	if(!this.running && (this.time > 0 || this.mode !== Timekeeper.Mode.TIMER)) {
